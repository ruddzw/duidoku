...
export default class GameBoard {
  ...
  isMoveAtIndexPossible (move, [indexOfGroup, indexWithinGroup]) {
    ...
    return this._indexesToConsider([indexOfGroup, indexWithinGroup]).every(([i, j]) => {
  ...
  _indexesToConsider ([indexOfGroup, indexWithinGroup]) {
    ...
    tests.forEach((test) => {
      const toConsiderFromTest = this._indexesToConsiderForTest([indexOfGroup, indexWithinGroup], test)
      toConsider = toConsider.concat(toConsiderFromTest)
    })

    const toConsiderUnique = unique(toConsider.map((i) => this.doubleIndexToIndex(i)))
  ...
  _indexesToConsiderForTest ([indexOfGroup, indexWithinGroup], test) {
    ...
    this.numRange.forEach((i) => {
      this.numRange.forEach((j) => {
        ...
        if (valueForOtherIndex === valueForIndex && (i !== indexOfGroup || j !== indexWithinGroup)) {
          toConsider.push([i, j])
        }
      })
    })
