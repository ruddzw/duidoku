export default class RandomOpponent {
  ...
  pickMove() {
    while (true) {
      ...
      const indexWithinGroup = Math.floor(
        Math.random() * this.gameBoard.fullNum
      )
      if (
        this.gameBoard.isIndexEmptyAndPossible([indexOfGroup, indexWithinGroup])
      ) {
        while (true) {
          // Find a move that is possible
          const move = Math.ceil(Math.random() * this.gameBoard.fullNum)
          if (
            this.gameBoard.isMoveAtIndexPossible(move, [
              indexOfGroup,
              indexWithinGroup
            ])
          ) {
